...
router.post('/leave-requests', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const { data, error } = await supabase
    .from('leave_requests')
    .insert({ ...body, school_id, user_id: req.user!.id })
...
router.patch('/leave-requests/:id', asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  if (result.completed && result.instance.status === 'approved') {
    ...
    if (balance) {
      await supabase.from('leave_balances')
        .update({ used_days: balance.used_days + leaveReq.total_days })
        .eq('id', balance.id)
    } else {
      ...
      await supabase.from('leave_balances').insert({
        ...
        total_days: lt?.default_days_per_year ?? 0, used_days: leaveReq.total_days,
      })
    }
  }
  ...
  res.json({
    success: true,
    data: { ...data, workflow_instance: result.instance, completed: result.completed },
  })
}))
