...
import { ArrowLeft, FileText, Phone, Mail, Send, Users, Search } from 'lucide-react'
...
// Same purpose-built lookup as /hr/recruitment/joined, one stage
// earlier in the pipeline: candidates an offer has gone out to but who
// haven't been marked Joined yet — filter by position, jump straight
// to the offer letter that was sent.
export default function OfferSentCandidatesPage() {
  ...
  const { data: offered, isLoading } = useQuery({
    queryKey: ['applications', 'offer_sent', jobFilter, search],
    queryFn: () => hrmsApi.applications.list({ status: 'offer_sent', job_posting_id: jobFilter || undefined, search: search || undefined }).then(r => r.data),
  })
    ...
    <div className="space-y-6">
      ...
      <Card className="overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            icon={Users}
            title={jobFilter || search ? 'No matching candidates' : 'No offers pending'}
            description={jobFilter || search ? 'Clear the search or position filter to see all offer-sent candidates.' : 'Candidates show up here once a selected candidate is approved and their offer is sent.'}
          />
        ) : (
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="hidden sm:table-cell">Offer Sent On</TableHead>
                  <TableHead />
                </TableRow>
              </TableHeader>
              <TableBody>
